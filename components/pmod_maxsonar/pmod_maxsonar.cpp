...
void PmodMaxSonar::read_data() {
  ...
  if (read_data[0] != 'R') {
    ESP_LOGW(TAG, "Got faulty reading from MAXSONAR, first char isn't 'R', instead got char id '%d'", read_data[0]);
   ...
   if (read_data[4] != '\r') {
    ESP_LOGW(TAG, "Got faulty reading from MAXSONAR, last char isn't '\\r', instead got char id '%d'", read_data[4]);
    this->publish_state(NAN);
    this->status_set_warning();
    return;
   }
